...
## Чому цей проєкт відповідає вимогам завдання

У завданні вимагалося:

- три різні бази даних;
- чотири ролі працівників;
- обчислення зарплати для кожної ролі;
- створення двох нових відділів;
- використання `Factory Method`, `Prototype`, `Singleton`, `Builder`;
- багатошарова архітектура;
- можливість розширення проєкту.

У цьому проєкті всі ці ідеї реалізовані.

Зокрема:

- присутні три джерела даних;
- створення об’єктів інкапсульоване у фабриках і builder;
- клонування використовується для створення одного нового відділу;
- singleton використовується для спільної відповідальності за підключення до БД;
- бізнес-аналітика винесена в окремий сервісний шар;
- код організований у шари;
- додавання нових типів баз даних і нових ролей працівників підтримується значно краще, ніж у жорстко захардкодженому рішенні.

## Короткий текст для захисту

Якщо потрібно коротко пояснити проєкт на здачі, можна сказати так:

Цей проєкт побудований за принципом багатошарової архітектури. Шар `data` працює з SQL Server, MongoDB та Excel. Шар `models` містить сутності працівників і логіку обчислення зарплати. Шар `factories` відповідає за вибір конкретної бази даних і створення об’єктів працівників. Патерн `Prototype` використовується для створення одного нового відділу клонуванням існуючих працівників, `Builder` використовується для створення іншого відділу, а `Singleton` відповідає за спільну логіку підключення до баз даних. Шар `services` виконує аналітику, наприклад обчислення середньої зарплати та статистики по відділах.

## Примітки

- Якщо `employees.xlsx` відкритий у Microsoft Excel, програма може не зчитати або не записати файл.
- Сценарії для SQL Server і MongoDB потребують локально запущених серверів баз даних.
- Для демонстрації на захисті найзручніше використовувати режим `excel`.
